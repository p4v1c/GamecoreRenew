...
def test_a_game_bezel_wins_over_the_system_one(tmp_path, monkeypatch):
    from backend.routers import overlays as ov
    monkeypatch.setattr(ov, "OVERLAYS_DIR", tmp_path)
    (tmp_path / "gopher64.png").write_bytes(b"\x89PNG\r\n\x1a\nSYSTEM")
    (tmp_path / "gopher64").mkdir()
    (tmp_path / "gopher64" / "Mario Kart 64.png").write_bytes(b"\x89PNG\r\n\x1a\nGAME")

    assert ov._resolve("gopher64", "Mario Kart 64.z64").read_bytes().endswith(b"GAME")
    # A game with no bezel of its own still gets the system's.
    assert ov._resolve("gopher64", "Zelda OOT.z64").read_bytes().endswith(b"SYSTEM")
    assert ov._resolve("nosuch", "Whatever.z64") is None


def test_the_stem_is_the_key_not_the_filename(tmp_path, monkeypatch):
    """Same key the covers and metadata caches use, so one game means one entry
    whatever extension it happens to carry."""
    from backend.routers import overlays as ov
    monkeypatch.setattr(ov, "OVERLAYS_DIR", tmp_path)
    a = ov._overlay_path("gopher64", "Mario Kart 64.z64")
    b = ov._overlay_path("gopher64", "Mario Kart 64.n64")
    assert a == b == tmp_path / "gopher64" / "Mario Kart 64.png"


def test_a_traversing_name_is_refused_not_sanitised(tmp_path, monkeypatch):
    """Silently rewriting a name would store the bezel under a key nothing else
    uses, and the game would never get it back."""
    from fastapi import HTTPException
    from backend.routers import overlays as ov
    monkeypatch.setattr(ov, "OVERLAYS_DIR", tmp_path)
    for bad in ("../../etc/passwd", "..", "a/b", "", "x" * 200):
        try:
            ov._overlay_path("gopher64", bad + ".z64")
    ...
    for bad in ("../etc", "a/b"):
        try:
            ov._overlay_path(bad)
        except HTTPException:
            pass
        else:
            raise AssertionError(f"system {bad!r} was accepted")
