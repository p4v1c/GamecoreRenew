...
if [[ "$MODE" == "full" ]]; then
  ...
  apt-get install -y wine unzip p7zip-full && ok "wine + archive tools installed." || warn "wine install failed."
  ...
  { apt-get install -y firefox-esr || apt-get install -y firefox; } && ok "firefox installed." || warn "firefox install failed."
